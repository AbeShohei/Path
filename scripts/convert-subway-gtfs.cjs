/**
 * Convert Kyoto Subway GTFS shapes.txt to JSON format for map display
 * 
 * This script:
 * 1. Reads shapes.txt to get polyline coordinates grouped by shape_id
 * 2. Reads routes.txt to get route names and colors
 * 3. Reads trips.txt to map shape_id to route_id
 * 4. Outputs kyoto-subway-routes.json with all routes
 */

const fs = require('fs');
const path = require('path');

const GTFS_DIR = path.join(__dirname, '../opendata/kyoto_subway_gtfs');
const OUTPUT_FILE = path.join(__dirname, '../public/data/kyoto-subway-routes.json');

// Parse CSV file to array of objects
function parseCSV(filePath) {
    const content = fs.readFileSync(filePath, 'utf-8');
    const lines = content.trim().split(/\r?\n/);
    const headers = lines[0].split(',').map(h => h.replace(/"/g, '').trim());

    return lines.slice(1).map(line => {
        // Handle quoted fields with commas
        const values = [];
        let current = '';
        let inQuotes = false;

        for (const char of line) {
            if (char === '"') {
                inQuotes = !inQuotes;
            } else if (char === ',' && !inQuotes) {
                values.push(current.trim());
                current = '';
            } else {
                current += char;
            }
        }
        values.push(current.trim());

        const obj = {};
        headers.forEach((h, i) => {
            obj[h] = values[i]?.replace(/"/g, '') || '';
        });
        return obj;
    });
}

// Main conversion function
function convertGTFStoJSON() {
    console.log('Reading Subway GTFS files...');

    // 1. Parse routes.txt
    const routes = parseCSV(path.join(GTFS_DIR, 'routes.txt'));
    const routeMap = new Map();
    routes.forEach(r => {
        routeMap.set(r.route_id, {
            routeId: r.route_id,
            routeShortName: r.route_short_name,
            routeName: r.route_long_name,
            color: r.route_color ? `#${r.route_color}` : '#FF0000', // Default red if missing
            textColor: r.route_text_color ? `#${r.route_text_color}` : '#FFFFFF'
        });
    });
    console.log(`Loaded ${routeMap.size} routes:`, Array.from(routeMap.values()).map(r => r.routeShortName));

    // 2. Parse trips.txt to map shape_id -> route_id
    const trips = parseCSV(path.join(GTFS_DIR, 'trips.txt'));
    const shapeToRoute = new Map();
    trips.forEach(t => {
        if (t.shape_id && !shapeToRoute.has(t.shape_id)) {
            shapeToRoute.set(t.shape_id, t.route_id);
        }
    });
    console.log(`Loaded ${shapeToRoute.size} shape-to-route mappings`);

    // 3. Parse shapes.txt and group by shape_id
    console.log('Parsing shapes.txt...');
    const shapesContent = fs.readFileSync(path.join(GTFS_DIR, 'shapes.txt'), 'utf-8');
    const shapeLines = shapesContent.trim().split(/\r?\n/).slice(1);

    const shapeCoords = new Map();

    shapeLines.forEach(line => {
        const [shape_id, lat, lon, seq] = line.split(',');
        const cleanShapeId = shape_id.replace(/"/g, '');

        if (!shapeCoords.has(cleanShapeId)) {
            shapeCoords.set(cleanShapeId, []);
        }

        shapeCoords.get(cleanShapeId).push({
            seq: parseInt(seq),
            lat: parseFloat(lat),
            lon: parseFloat(lon)
        });
    });
    console.log(`Loaded ${shapeCoords.size} unique shapes`);

    // 4. Build final route data
    const routeData = [];
    const processedRoutes = new Set();

    // We want to verify we get all routes, so let's iterate over shapeCoords
    // Note: GTFS shapes often have directions. For visualization, we might want both or merged.
    // For subway, let's keep all unique shapes for now but maybe filter by direction if needed.
    // Actually, usually one shape per direction per route.

    shapeCoords.forEach((coords, shapeId) => {
        const routeId = shapeToRoute.get(shapeId);
        if (!routeId) return;

        const routeInfo = routeMap.get(routeId);
        if (!routeInfo) return;

        // Strategy: We want to show the full network. 
        // If we just pick one shape per routeId, we might miss branches or the other direction if it's different.
        // But for Kyoto subway (simple lines), typically shapes are identical just reversed.
        // Let's deduce uniqueness by routeId and rough length to avoid duplicates?
        // Or simple approach: Just add ALL shapes, but verify visuals.
        // Better approach for simple visual: One shape per route.
        // Kyoto subway is simple enough. Let's try to get one shape per routeId.

        if (processedRoutes.has(routeId)) return;
        processedRoutes.add(routeId);

        // Sort by sequence
        coords.sort((a, b) => a.seq - b.seq);
        const coordinates = coords.map(c => [c.lat, c.lon]);

        routeData.push({
            routeId: routeInfo.routeId,
            routeName: routeInfo.routeName,
            routeShortName: routeInfo.routeShortName,
            color: routeInfo.color,
            description: routeInfo.routeName, // e.g. "Tozai Line"
            coordinates: coordinates
        });
    });

    console.log(`Generated ${routeData.length} unique route shapes`);

    // 5. Write output file
    const output = {
        generatedAt: new Date().toISOString(),
        source: 'ODPT Kyoto Municipal Subway GTFS',
        routeCount: routeData.length,
        routes: routeData
    };

    fs.writeFileSync(OUTPUT_FILE, JSON.stringify(output, null, 2), 'utf-8');
    console.log(`Output written to ${OUTPUT_FILE}`);
}

convertGTFStoJSON();
