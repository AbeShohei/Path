...
**1. 時間帯区分 (Time Divisions)**
観光客の行動パターンに合わせ、データを以下の3つの時間帯に分割して分析しています。

- **朝 (06:00-12:00)**: 観光開始前の移動時間帯
- **昼 (12:00-18:00)**: 最も混雑するピークタイム
- **夜 (18:00-24:00)**: 観光終了後の夕食・移動時間帯

**2. 京都市内エリアの判定基準 (Regional Deviation Scores)**
常に人が多い京都市内（北緯34.85-35.12, 東経135.55-135.90）では、**偏差値（T-score）**を用いた判定を採用しています。人口データを対数変換（Log-normal）して正規分布に近づけた上で、標準偏差に基づいて分類します。

| レベル | 偏差値 (T) | 判定 | 割合の目安 |
| :--- | :--- | :--- | :--- |
| **Level 1** | **T < 35** | 非常に空いている | 下位 6.7% |
| **Level 2** | **35 ≦ T < 45** | 空いている | 次の 24.2% |
| **Level 3** | **45 ≦ T < 55** | 通常 (平均的) | 中間 38.2% |
| **Level 4** | **55 ≦ T < 65** | 混雑 | 次の 24.2% |
| **Level 5** | **65 ≦ T** | 非常に混雑 | 上位 6.7% |

※市外エリアについては、従来通りパーセンタイル（20%刻み）による判定を行っています。
...
- **System Instruction**:
   > あなたは京都に精通した「AI観光ガイド」です。ユーザーの移動中に、その場所の歴史、文化、風景の魅力を語ってください。車内アナウンスのような事務的な内容は最小限にし、車窓から見える景色や、その地域の知られざるエピソードを情緒豊かに解説してください。
- **Stage別タスク**:
  - `TO_STOP` (バス停へ): 「これから始まる旅への期待を高めるような、街歩きの楽しみ方を軽く語る」
  - `ON_BUS` (乗車中): 「単なる移動時間を『観光の時間』に変えるような、深みのある解説（沿線の歴史や風物詩）」
  - `TO_DEST` (目的地へ): 「目的地の歴史的背景、見どころ、参拝のポイントを詳しく解説」
